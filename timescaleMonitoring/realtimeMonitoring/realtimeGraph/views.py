...
def get_station_summary_json(request):
    start, end = get_daterange(request)
    measurement_param = request.GET.get("measurement", None)
    ...
    if measurement_param:
        data = data.filter(measurement__name=measurement_param)

    weighted_avg_expr = ExpressionWrapper(
    ...
    summary = (
        ...
        .annotate(
            bucket_count=Count("time"),
            readings_count=Sum("length"),
            min_value=Min("min_value"),
            max_value=Max("max_value"),
            latest_time=Max("base_time"),
            weighted_sum=Sum(weighted_avg_expr),
        )
        .order_by("-readings_count")[:limit]
    )
    ...
    for row in summary:
        ...
        avg_value = (weighted_sum / readings_count) if readings_count > 0 else 0
        results.append(
            {
                "station_id": row["station_id"],
                "user": row["station__user__login"],
                "location": {
                ...
                "min_value": row["min_value"] if row["min_value"] is not None else 0,
                "max_value": row["max_value"] if row["max_value"] is not None else 0,
    ...
    return JsonResponse(
        {
            ...
            "to": end.isoformat(),
            "measurement_filter": measurement_param,
            "limit": limit,
            "count": len(results),
            "results": results,
        }
